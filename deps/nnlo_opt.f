...
c******************************************************************
c
c        This code computes the OPTIMIZED
c        charge-dependent chiral NN potential at
...
c        only `nnlo' needs to be called by the user.
...
c*******************************************************************
c
c        The optimized chiral NNLO potential is published in:
c
c        A. Ekstom, G. Baardsen, C. Forssen, G. Hagen, M. Hjorth-Jensen,
c        G. R. Jansen, R. Machleidt, W. Nazarewicz, T. Papenbrock,
c        J. Sarich, and S. M. Wild, arXiv:1303.4674 [nucl-th].
c
c*******************************************************************
c*******************************************************************
c
...
c        choose the np potential as the default:
      ...
      if (j.lt.0) then
      write (kwrite,19002)
19002 format (////' error in nnlo: total angular momentum j',
...
c10001 format (' The pp potential is used.')
c      go to 24
c   22 write (kwrite,10002)
c10002 format (' The np potential is used.')
c      go to 24
c   23 write (kwrite,10003)
c10003 format (' The nn potential is used.')
...
c       inversion
c
      call dminv_opt (adminv,4,deter,ldminv,mdminv)
c
c
c
c
...
19000 format(1h ////' warning in  nnlo: contribution ',a4,' does not exi
...
c
c
      data ((tab(i,j),i=1,5),j=1,42)/
     1    6.000000d0,   0.0d0,    3.0000d0,  500.0d0,   0.0d0,
     ...
     7    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     8    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
...
c
     ...
     *   55.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     ...
     7   -0.152141088d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     8    2.404021944d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     9    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     *    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     1    1.263390763d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     2    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     3    0.417045542d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     4    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     5   -0.782658500d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     6    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     7   -0.158434177d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     8    0.928384663d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     9    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     *    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     1    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     2    0.618141419d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     3    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     4    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     5    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     6    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     7   -0.677808511d0,0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     8    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     9    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     *    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0,
     1    0.000000d0,   0.0d0,    0.0000d0,    0.0d0,   0.0d0/
...
c
      data t1s0pp/
     7   -0.151366037d0, 0.0d0,    0.0000d0,    0.0d0,   0.0d0/
c
c
      data t1s0np/
     7   -0.152141088d0, 0.0d0,    0.0000d0,    0.0d0,   0.0d0/
...
c
      data t1s0nn/
     7   -0.151764746d0, 0.0d0,    0.0000d0,    0.0d0,   0.0d0/
c
c
...
c10008 format (1h ,63(1h-))
10010 format (1h ,2a4,a2,i3,2f10.2)
c10011 format (//' nnlo: Charge-Dependent Chiral NN Potential',
c     1 ' at Order Three(NNLO)')
...
c
c
c        fix index-parameter concerning the over-all factor
c        and the sfr cutoff.
c
      ift(inter)=1
      ezz1(inter)=0.d0
c        the following line states the sfr cutoff.
      ezz2(inter)=700.d0
c
...
c
c
      call dgelg_opt (b,a,7,1,eps,ier)
c
...
c
c
      call dgelg_opt (b,a,15,1,eps,ier)
c
...
c
 9999 stop
      ...
      subroutine chistr_opt (icase,max,mex)
c
...
c
c
      call gset_opt (-1.d0,1.d0,nt,ct,wt)
      nnt=nt
c
c
c
...
c
      ...
      do 99 i=1,nt
      t=ct(i)
      call legp_opt (pj(1,i),pj(3,i),t,j)
      pj(2,i)=pj(1,i)*t
      pj(4,i)=pj(2,i)*t
      pj(6,i)=pj(4,i)*t
      pj(5,i)=pj(3,i)*t
...
c
 8000 return
      ...
      subroutine legp_opt (pj,pjm1,x,j)
c
c
...
c
      ...
      subroutine gset_opt(ax,bx,n,z,w)
c
c
...
c
c**********************************************************************
      subroutine dgelg_opt(r,a,m,n,eps,ier)
c
c
      implicit real*8 (a-h,o-z)
      dimension a(1),r(1)
...
c
c**********************************************************************
      subroutine dminv_opt (a,n,d,l,m)
      implicit real*8 (a-h,o-z)
